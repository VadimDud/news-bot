...
from bot.asset_analyzer import analyze_ticker
...
class TestAnalyzeTicker:
    async def test_deepseek_success(self):
        ...
        with patch("bot.asset_analyzer.analyze", new_callable=AsyncMock, return_value=mock_result):
            result = await analyze_ticker("SBER")
            assert result["ticker"] == "SBER"
            assert result["company_name"] == "Сбербанк"

    async def test_fallback_when_all_fail(self):
        with patch("bot.asset_analyzer.analyze", new_callable=AsyncMock, return_value=None):
    ...
    async def test_missing_fields_default(self):
        ...
        with patch("bot.asset_analyzer.analyze", new_callable=AsyncMock, return_value=mock_result):
            result = await analyze_ticker("SBER")
            assert result["positive_triggers"] == ["дивиденды", "рост", "прибыль", "рекорд", "апгрейд"]
            assert result["negative_triggers"] == ["падение", "убыток", "санкции", "расследование", "дефолт"]
