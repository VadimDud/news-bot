...
⚠️ ИСТОРИЯ (2026-09): прод-бэктест elliott_candles.run_backtest содержал
LOOKAHEAD-БАГ. detect_waves считал волну завершённой только когда следующая
свеча уже ДРУГОГО цвета — и вход делался на open этой подтверждающей свечи,
чьё направление (close>open или close<open) УЖЕ ИЗВЕСТНО по построению.
Fade-направление противоположно волне == совпадало с направлением подтверждающей
свечи -> win 98.8%, «прибыль» — тавтология. В live цвет следующей свечи неизвестен.

ИСПРАВЛЕНО (2026-09): run_backtest переписан на live-faithful (сигнал на закрытии
свечи, завершающей волну L=3..5, вход на open следующей, додзи торгуются).
Этот скрипт остаётся эталонной проверкой: результаты должны совпадать с продом.
...
Прод-бэктест исправлен (2026-09): цифры прод-дашборда теперь совпадают
с live-faithful моделью этого скрипта.
