...
def format_elliott_signal(
    ...
    # Рекомендуемый план = выигрышный вариант E6 (hold-add, только лонг).
    if wave.direction == "bear":
        plan = (
            f"План: вход на открытии следующего дня (только лонг).\n"
            f"При минусе на закрытии — держим и добавляем лот на open след. дня "
            f"(25%→50%→остаток 25%; суммарно не более 100%); выход при суммарном "
            f"плюсе или после 3 добавок."
        )
    else:
        plan = f"План: {plan}."
    ...
    return (
        ...
        f"⚠️ Усреднение вниз (hold-add, аналог мартингейла) — высокий риск."
    )
