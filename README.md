...
- **Скачивание данных**: карточка на главной — свечи MOEX любого тикера в CSV
  ...
  информация — хвост после последней сохранённой свечи и недостающий бэкфилл
  (upsert по ключу ticker+period+begin). По умолчанию таймфрейм — 1 минута.
  Ненативные таймфреймы (5/15/30 min) ресэмплятся из 1-минутных свечей.
- **Риск-менеджмент** (общие параметры каждой стратегии в форме бэктеста):
  риск на сделку в % (`risk_pct`, 1–2%), стоп-лосс по ATR (`atr_stop_mult`),
  тейк-профит с соотношением R:R (`rr_ratio`), трендовый фильтр EMA
  (`trend_period` — покупки только выше скользящей). Лот считается автоматически:
  `риск-сумма / дистанция стопа`, с ограничением свободными деньгами.
- **Live**: цикл опроса котировок T-Bank, сигнал по выбранной стратегии, ордера
  через `OrdersService`, тот же риск-менеджмент (размер позиции от equity,
  слежение за стоп-лоссом/тейк-профитом). По умолчанию **dry-run**
  (`TRADER_DRY_RUN=true`) — реальные ордера выставляются только при явном
  отключении.
